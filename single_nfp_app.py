...
if st.button("🎨 Generate Nutrition Facts Panel", type="primary", use_container_width=True):
    ...
    # Create the visualization - OPTIMIZED FOR STREAMLIT
    fig = plt.figure(figsize=(4.8, 10.5), dpi=100, facecolor='white')
    ax = fig.add_subplot(111)
    ax.set_xlim(0, 10)
    ax.set_ylim(0, 16)
    ax.axis('off')
    
    # Positioning - adjusted for better rendering
    left_margin = 0.6
    right_margin = 9.4
    
    def add_text(x, y, text, size=10, weight='normal', ha='left'):
        ax.text(x, y, text, fontsize=size, weight=weight, ha=ha, va='top', 
                family='DejaVu Sans', antialiased=True)
    
    def draw_line(y, thickness=1, style='solid'):
        ax.plot([left_margin, right_margin], [y, y], 'k-', 
                linewidth=thickness, linestyle=style, solid_capstyle='butt')
    ...
    # Outer border - fixed to prevent overflow
    border = Rectangle((left_margin - 0.15, 0.35), 
                       right_margin - left_margin + 0.3, 
                       y_pos - 0.25, 
    ...
    # Display in Streamlit - OPTIMIZED
    ...
    fig_download = plt.figure(figsize=(4.8, 10.5), dpi=300, facecolor='white')
    ax_download = fig_download.add_subplot(111)
    ax_download.set_xlim(0, 10)
    ax_download.set_ylim(0, 16)
    ax_download.axis('off')
    
    # Redraw for high-res download (same code as above)
    y_pos = 15.5
    border = Rectangle((left_margin - 0.15, 0.35), 
                       right_margin - left_margin + 0.3, 
                       y_pos - 0.25, 
                       linewidth=2, edgecolor='black', facecolor='none',
                       joinstyle='miter', capstyle='butt')
    ax_download.add_patch(border)
    
    def add_text_dl(x, y, text, size=10, weight='normal', ha='left'):
        ax_download.text(x, y, text, fontsize=size, weight=weight, ha=ha, va='top', 
                        family='DejaVu Sans', antialiased=True)
    
    def draw_line_dl(y, thickness=1, style='solid'):
        ax_download.plot([left_margin, right_margin], [y, y], 'k-', 
                        linewidth=thickness, linestyle=style, solid_capstyle='butt')
    
    y_pos = 15.5
    add_text_dl(left_margin, y_pos, 'Nutrition Facts', size=32, weight='bold')
    y_pos -= 0.8
    add_text_dl(left_margin, y_pos, f'{servings_per_container} servings per container', size=9)
    y_pos -= 0.35
    add_text_dl(left_margin, y_pos, 'Serving size', size=9, weight='bold')
    add_text_dl(left_margin + 2.5, y_pos, serving_size, size=9, weight='bold')
    y_pos -= 0.35
    draw_line_dl(y_pos, thickness=12)
    y_pos -= 0.45
    add_text_dl(left_margin, y_pos, 'Amount per serving', size=8, weight='bold')
    y_pos -= 0.3
    add_text_dl(left_margin, y_pos, 'Calories', size=15, weight='bold')
    add_text_dl(right_margin, y_pos, str(cal_val), size=42, weight='bold', ha='right')
    y_pos -= 1.0
    draw_line_dl(y_pos, thickness=6)
    y_pos -= 0.35
    add_text_dl(right_margin, y_pos, '% Daily Value*', size=8, weight='bold', ha='right')
    y_pos -= 0.25
    draw_line_dl(y_pos, thickness=1)
    y_pos -= 0.35
    ...
    for nutrient in ['Total Fat', 'Saturated Fat', 'Trans Fat', 'Cholesterol', 'Sodium',
        ...
        amount_str = format_amount(nutrient, amount, unit)
        add_text_dl(indent, y_pos, display_name, size=size, weight=weight)
        add_text_dl(right_margin - 1.5, y_pos, amount_str, size=size, weight='normal', ha='right')
        
        if dv is not None and nutrient not in ['Trans Fat', 'Total Sugars']:
            add_text_dl(right_margin, y_pos, f'{int(dv)}%', size=11, weight='bold', ha='right')
    ...
    y_pos -= 0.2
    add_text_dl(left_margin, y_pos, footer_text, size=8, weight='normal')
    
    plt.subplots_adjust(left=0.05, right=0.95, top=0.98, bottom=0.02)
    
    buf = BytesIO()
    fig_download.savefig(buf, format='png', dpi=300, bbox_inches='tight', 
